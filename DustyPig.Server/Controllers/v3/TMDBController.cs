...
internal class TMDBController : _BaseProfileController
{
    ...
    public TMDBController(AppDbContext db, TMDBService tmdbService) : base(db)
    {
    ...
    public async Task<Result<DetailedTMDB>> GetMovie(int id)
    {
        ...
        var movieResponse = await _tmdbService.GetMovieAsync(id);
        ...
        {
            ArtworkUrl = TMDBService.GetPosterPath(movie.PosterPath),
            BackdropUrl = TMDBService.GetBackdropPath(movie.BackdropPath),
            Description = movie.Overview,
            MediaType = TMDB_MediaTypes.Movie,
            Rated = TMDBService.TryMapMovieRatings(movie),
            Title = movie.Title,
            TMDB_ID = movie.Id
        };

        ret.Year = TMDBService.TryGetMovieDate(movie)?.Year ?? 0;

        if (movie.Genres != null)
            ret.Genres = string.Join(",", movie.Genres.Select(item => item.Name)).ToGenres();

        FillCredits(TMDBService.GetCommonCredits(movie), ret);
    ...
    public async Task<Result<DetailedTMDB>> GetSeries(int id)
    {
        ...
        var seriesResponse = await _tmdbService.GetSeriesAsync(id);
        ...
        var ret = new DetailedTMDB
        {
            ArtworkUrl = TMDBService.GetPosterPath(series.PosterPath),
            BackdropUrl = TMDBService.GetBackdropPath(series.BackdropPath),
            Description = series.Overview,
            MediaType = TMDB_MediaTypes.Series,
            Rated = TMDBService.TryMapTVRatings(series),
            Title = series.Name,
            TMDB_ID = series.Id
        };
        ...
        FillCredits(TMDBService.GetCommonCredits(series), ret);
        ret.Credits?.ForEach(c =>
        {
            if (string.IsNullOrWhiteSpace(c.AvatarUrl))
                c.AvatarUrl = Constants.DEFAULT_PROFILE_IMAGE_GREY;
    ...
    {
        var response = await _tmdbService.Endpoints.People.GetDetailsAsync(id, TMDB.Models.People.AppendToResponse.CombinedCredits);
        if (!response.Success)
            return response.Error.Message;

        var ret = new API.v3.Models.TMDB_Person
        {
            AvatarUrl = TMDBService.GetAvatarPath(response.Data.ProfilePath),
        ...
        if (response.Data.CombinedCredits != null)
        {
            ...
            foreach (var item in castQ)
            {
                ...
                if (!exists)
                {
                    ...
                    {
                        ArtworkUrl = TMDBService.GetPosterPath(item.PosterPath),
                        BackdropUrl = TMDBService.GetBackdropPath(item.BackdropPath),
            ...
            {
                if (TMDBService.CrewJobs.ICContains(item.Job))
                {
                    var mt = item.MediaType == TMDB.Models.Common.CommonMediaTypes.Movie ? MediaTypes.Movie : MediaTypes.Series;
                    bool exists = castList
                        .Where(_ => _.Id == item.Id)
    ...
    public async Task<Result> RequestTitle(TitleRequest data)
    {
        ...
        {
            var response = await _tmdbService.GetMovieAsync(data.TMDB_Id);
            if (!response.Success)
                return CommonResponses.ValueNotFound(nameof(data.TMDB_Id));
            title = response.Data.Title;
            var dt = TMDBService.TryGetMovieDate(response.Data);
            if (dt.HasValue)
                title += $" ({dt.Value.Year})";
        }
        else
        {
            var response = await _tmdbService.GetSeriesAsync(data.TMDB_Id);
            if (!response.Success)
                return CommonResponses.ValueNotFound(nameof(data.TMDB_Id));
